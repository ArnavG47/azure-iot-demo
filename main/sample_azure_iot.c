 ...
 */
#define sampleazureiotMESSAGE   "{ \"deviceId\": \"%s\", \"buttonCount\": %u }"
...
/*
            ...
            //   ESP_LOGI(TAG_LED_TASK, "button clicked");
...
*/
            ...
            configASSERT( xResult == eAzureIoTSuccess );
            LogInfo(("attempting to append properties \r\n"));
            
            char cnt_str[12];
            int n = snprintf(cnt_str, sizeof cnt_str, "%lu", (unsigned long)g_button_count);

            /* How to send an user-defined custom property. */
            xResult = AzureIoTMessage_PropertiesAppend( &xPropertyBag, ( uint8_t * ) "Clicks", sizeof( "Clicks" ) - 1,
                                                        ( uint8_t * ) cnt_str, (uint32_t)n );
            configASSERT( xResult == eAzureIoTSuccess );
            LogInfo(("appended properties \r\n"));
                 ...
                 lPublishCount++ )
            {
                ulScratchBufferLength = snprintf( ( char * ) ucScratchBuffer, sizeof( ucScratchBuffer ),
                                                  sampleazureiotMESSAGE, (const char*)pucIotHubDeviceId,(unsigned int)g_button_count);
